...
def load_backend_registry(path: Path | None = None) -> dict:
    ...
    project_status = json.loads((source.parent / "project_status.json").read_text())
    validate_backend_registry(registry, project_status, set(IMPLEMENTED_BACKENDS))
    return registry


def validate_backend_registry(registry: dict, project_status: dict, cli_backends: set[str] | None = None) -> None:
