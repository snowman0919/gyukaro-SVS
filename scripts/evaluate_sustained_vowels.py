...
def main() -> None:
    ...
    renderer = HybridRenderer(load_hybrid_model("checkpoints/gyu_hybrid_v0.2.pt", device), MossCodecDecoder("data/cache/moss-audio-tokenizer-nano", device), "data/processed/master/216.wav")
    ...
    for language, lyric in VOWELS.items():
        ...
        path = Path(f"artifacts/samples/hybrid_sustain_{language}.wav")
    ...
    Path("artifacts/reports/hybrid_sustained_vowels.json").write_text(json.dumps(output, indent=2) + "\n")
