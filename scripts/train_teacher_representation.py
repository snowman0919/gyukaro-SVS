...
def main() -> None:
    ...
    for step in range(1, 301):
        moss_row, fish_row = paired[(step - 1) % len(paired)]; m = torch.load(moss_row["path"], weights_only=True).to(device)[None]; f = torch.load(fish_row["path"], weights_only=True).to(device)[None]; shared = model(f, m); ref = acoustic_reference_features("data/processed/master/216.wav").to(device)[None]; student = model.student(ref); trust = torch.tensor([min(float(moss_row.get("trust_weight", .2)), float(fish_row.get("trust_weight", .2)))], device=device); loss = (1 - torch.nn.functional.cosine_similarity(shared, student).mean()) * trust
    ...
    report = {"teachers": ["fish_s2_pro", "moss_local_v15"], "paired_rows": len(paired), "representation_shapes": {"fish": [1024], "moss": [768], "shared": [32]}, "loss": "trust_weight * cosine(student_reference, shared_projected_teacher)", "trust_weight": "minimum paired manifest trust_weight scales objective", "gradient_evidence": history, "checkpoint": output, "notes": "Fish DAC and MOSS tokenizer are real internal neural representations; no waveform summary used for this path."}
