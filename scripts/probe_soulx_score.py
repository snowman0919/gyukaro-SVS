...
def render(model, config, reference, ref_f0, source_path: str, contour: np.ndarray, output: str) -> None:
...
def worker(model, config, reference, ref_f0) -> None:
    for line in sys.stdin:
        try:
            ...
            render(model, config, reference, ref_f0, request["source"], np.load(request["f0_npy"]).astype(np.float32), request["output"])
