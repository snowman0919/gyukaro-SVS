...
def main() -> None:
    ...
    usable = [p for p in pairs if (p["benchmark_id"], "fish_s2_pro") in by_key and (p["benchmark_id"], "moss_local_v15") in by_key]
    ...
    for step in range(1, 801):
        ...
        loss = weight * ((1 - F.cosine_similarity(fish, moss).mean()) + 0.5 * (1 - F.cosine_similarity(shared, student).mean()))
    ...
    report = {"paired_rows": len(usable), "split_counts": {s: sum(p["split"] == s for p in usable) for s in ("train", "validation", "test")}, "language_counts": {l: sum(p["language"] == l for p in usable) for l in ("ko", "en", "ja")}, "shared_dim": 64, "teacher_representations": {"fish": "Fish-S2-Pro-DAC.encoder_hidden", "moss": "MOSS-Audio-Tokenizer-Nano.encoder_hidden_states"}, "history": history, "checkpoint": str(checkpoint), "negative_policy": "no fabricated speaker negatives; cross-view consistency only"}
