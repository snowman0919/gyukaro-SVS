#!/usr/bin/env python3
"""Measure whether CTC→WSOLA improves source/score timing before SoulX."""
...
def main() -> None:
    root = Path("artifacts/reports/rc5_content_timing"); manifest = json.loads((root / "manifest.json").read_text())
    ...
    for case, data in manifest["cases"].items():
        ...
        for label in ("source", "corrected"):
            path = Path(data[label]); rows.append({"case": case, "label": label, "path": str(path)} | acoustics(path) | pitch(path, target, extractor))
    del extractor; torch.cuda.empty_cache()
    processor = AutoProcessor.from_pretrained(CACHE / "whisper-large-v3-turbo"); model = AutoModelForSpeechSeq2Seq.from_pretrained(CACHE / "whisper-large-v3-turbo", dtype=torch.float16).cuda().eval()
    for row in rows:
    ...
    metrics = ("voicing_accuracy", "observed_voiced_ratio", "hf_energy_ratio_p95", "spectral_flatness_mean", "spectral_flux_p95", "sample_jump_p999", "asr_lyric_similarity")
    aggregate = {label: {name: round(float(np.mean([row[name] for row in rows if row["label"] == label])), 6) for name in metrics} for label in ("source", "corrected")}
    decision = "reject" if aggregate["corrected"]["asr_lyric_similarity"] < aggregate["source"]["asr_lyric_similarity"] - .02 or aggregate["corrected"]["voicing_accuracy"] <= aggregate["source"]["voicing_accuracy"] else "eligible_for_soulx_probe"
    report = {"status": "measured", "decision": decision, "aggregate": aggregate, "rows": rows, "warning": "source diagnostics only; human listening and downstream SoulX comparison still required"}
