...
def main() -> None:
    ...
    parser.add_argument("--root", type=Path, default=ROOT / "artifacts/reports/rc8_interval_candidate")
    args = parser.parse_args()
    root = args.root
    manifest = json.loads((root / "manifest.json").read_text())
    variants = [{"variant": "rc7", "strength": .5, "path": "artifacts/reports/rc7_listening_gate/08_large_interval_ko.wav"}] + [
        row | {"variant": row["variant"] if "variant" in row else f"s50_spectral_{row['strength']:g}"}
    ...
    for variant in variants:
        ...
        rows.append({"variant": variant["variant"], "strength": variant.get("strength"), "path": variant["path"]} | acoustics(path) | pitch(path, target, extractor) | {"failure_region": interval_f0(path)})
    del extractor
    ...
    resolved = [
        ...
        if abs(row["failure_region"]["pyin_median_hz"] - 387.3) < 100
        and row["failure_region"]["estimator_disagreement_p90_cents"] < 100
    ]
    selected = min(
        resolved,
        key=lambda row: (
            abs(row["failure_region"]["pyin_median_hz"] - 387.3),
            row["failure_region"]["estimator_disagreement_p90_cents"],
