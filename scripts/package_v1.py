...
(root / "README.md").write_text("# GYU Hybrid Singer v0.2-experimental\n\nCompact phrase-level neural SVS runtime. `sh install.sh`, then `sh run.sh`. Checkpoint has 1,200 CFM steps on 60 real anchors and 665 weighted teacher rows used only for representation loss. Fish S2 pseudo-singing probes are excluded from training after license review. Output quality is poor and multilingual singing is not claimed. This package has no source-loop, per-note TTS, pitch-shift, or phase-vocoder path.\n")
(root / "MODEL_CARD.md").write_text("## Scope\n\nInput: Korean, English, or Japanese lyric-note protocol-v2 JSON. Model: TriSinger conditional-flow acoustic-latent generator (3.0 MB checkpoint) decoded by frozen Apache-2.0 MOSS audio tokenizer. Training: 60 real train rows, 5 real validation, 5 real test; 665 teacher rows only for trust-weighted representation loss. Fish S2 pseudo-singing outputs are evaluation-only after license review. Real-anchor score timing is inferred from speech duration, not ground-truth singing notation.\n\nThis is an experimental personalized SVS runtime, not a production-quality multilingual singer. It emits phrase audio, but measured F0 and intelligibility do not yet meet a v1 bar.\n")
