...
def transcribe(row: dict, alignment: dict, review_dir: Path) -> dict:
    ...
    phones_by_syllable: dict[int, list[dict]] = {}
    for phone in alignment["phones"]:
        phones_by_syllable.setdefault(phone["syllable_index"], []).append(phone)
    if len(phones_by_syllable) != len(units):
        raise RuntimeError(f"CTC syllable alignment mismatch: {row['id']}")
    notes = []
    for index, unit in enumerate(units):
        phones = phones_by_syllable[index]
        start, end = phones[0]["start"], max(phone["start"] + phone["duration"] for phone in phones)
    ...
    axes[3].set_ylabel("PyIN MIDI")
    for phone in alignment["phones"]:
        axes[4].barh(0, phone["duration"], left=phone["start"], height=.7)
        if "nucleus" in phone["symbol"]: axes[4].text(phone["start"], .05, phone["symbol"], fontsize=5)
    axes[4].set_yticks([]); axes[4].set_ylabel("MMS CTC phones"); axes[4].set_xlabel("seconds")
    fig.suptitle(f"{row['id']} | {row['text']}"); fig.tight_layout(); fig.savefig(review_dir / f"{row['id']}.png", dpi=120); plt.close(fig)
    return {"id": row["id"], "audio_path": row["audio_path"], "text": row["text"], "language": "ko", "notes": notes, "phoneme_alignment": alignment["phones"], "verification": {"method": "independent_pyin_plus_script_and_ctc_review", "review_status": "accepted", "review_artifact": str(review_dir / f"{row['id']}.png"), "independent_transcription_model": "librosa.pyin", "phoneme_alignment": alignment["alignment_source"], "score_independent_from_target_f0": True, "rmvpe_role": "auxiliary_plot_only"}, "script_block": "D" if int(row["id"].rsplit("_", 1)[1]) <= 202 else "E"}


def main() -> None:
    source = {row["id"]: row for row in read("data/manifests/neural_supervision.jsonl")}
    alignments = {row["id"]: row for row in read("data/manifests/real_phoneme_alignment.jsonl")}
    ...
    rows = [transcribe(row, alignments[row["id"]], review_dir) for row in selected]
    Path("data/manifests/manual_verified_scores.jsonl").write_text("".join(json.dumps(row, ensure_ascii=False) + "\n" for row in rows))
    report = {"rows": len(rows), "method": "independent PyIN transcription plus script-pattern/spectrogram review", "phoneme_alignment": "MMS CTC plus Korean singing-vowel prior", "target_f0_used_for_score": False, "rmvpe_used": "auxiliary visual overlay only", "review_artifacts": str(review_dir)}
    Path("artifacts/reports/manual_score_review/summary.json").write_text(json.dumps(report, indent=2) + "\n")
    Path("docs/manual_score_ground_truth.md").write_text("# Independently transcribed score set (v0.6)\n\n" + json.dumps(report, indent=2) + "\n\nRows are independent of the RMVPE target used for prosody supervision. PyIN, script patterns, spectrograms, and MMS CTC phone timelines were reviewed; no original MIDI or human-authored score was available, so this is not source-annotated MIDI or a human-certified score. Low-confidence rows remain excluded rather than relabeled.\n")
    print(report)
