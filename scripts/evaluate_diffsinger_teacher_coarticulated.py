#!/usr/bin/env python3
"""Gate coarticulated Fish/MOSS teacher plus real-GYU score-native pilots."""
...
VARIANTS = {
    ...
    "gyu_coarticulated400": {
        case: LISTENING / f"{case}_coart400.wav" for case in CASES
    },
    **{
        f"teacher_coarticulated{step}": {
            case: LISTENING / f"{case}_teacher_coart{step}.wav" for case in CASES
        }
        for step in (400, 500, 600, 900, 1200)
    },
}
...
def main() -> None:
    ...
    candidates = tuple(model for model in VARIANTS if model.startswith("teacher_coarticulated"))
    candidate = max(
        candidates,
        key=lambda model: (
    ...
    step = int(candidate.removeprefix("teacher_coarticulated"))
    checkpoint = CACHE / f"diffsinger/checkpoints/gyu_score_native_teacher_coarticulated/model_ckpt_steps_{step}.ckpt"
    report = {
        "status": "objective_probe_pass_human_pending" if eligible else "objective_reject_teacher_coarticulation_insufficient",
        ...
        "training": {
            "real_gyu_rows": 52,
            "fish_teacher_rows": 30,
            "moss_teacher_rows": 30,
            "unique_teacher_texts": 30,
            "train_seconds": 523.62,
            "validation_seconds": 109.26,
            "teacher_sp_duration_ratio_before": 0.831603,
            "teacher_sp_duration_ratio_after": 0.142237,
            "real_gyu_sp_duration_ratio": 0.105781,
            "teacher_audio_role": "low-trust lexical/acoustic speech prior; not real GYU singing",
        },
        ...
        "interpretation": "Objective gates can reject this pilot; only later human review can pass a release candidate.",
    }
    (ROOT / "artifacts/reports/diffsinger_teacher_coarticulated_evaluation.json").write_text(
