...
class HybridRenderer:
    ...
    def batch(self, score: dict) -> dict[str, torch.Tensor]:
        ...
        frames = build_phrase_frames(phonemize(score["language"], text), score["notes"])
        controls = score["expressions"]
        style = torch.tensor([
            float(controls.get("dynamics", 0.8)), float(controls.get("breathiness", 0.0)),
            float(controls.get("tension", 0.0)), float(controls.get("brightness", 0.0)),
            float(controls.get("vibrato", 0.0)),
        ], device=self.device)
        return {
            ...
            "style_preset": torch.zeros(1, dtype=torch.long, device=self.device), "style_controls": style[None],
        }
