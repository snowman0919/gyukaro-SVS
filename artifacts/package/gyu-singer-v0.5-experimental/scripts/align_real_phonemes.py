...
def align(row: dict, model: torch.nn.Module, dictionary: dict[str, int], labels: tuple[str, ...], device: str) -> dict:
    ...
    for syllable_index, (unit, end) in enumerate(zip(units, ends)):
        ...
        # Singing-aware prior: consonants stay short; sustained vowel gets remainder.
        onset = min(.08, duration * .18) if len(output.symbols) > 1 else 0
        coda = min(.12, duration * .18) if len(output.symbols) == 3 else 0
    ...
    nucleus = sum(phone["duration"] for phone in phones if "nucleus" in phone["symbol"])
    total = sum(phone["duration"] for phone in phones)
    return {"id": row["id"], "alignment_source": "MMS_multilingual_CTC_forced_alignment_plus_singing_vowel_prior", "confidence": round(float(np.clip(.5 + .5 * (nucleus / max(total, 1e-6)), 0, 1)), 4), "uniform_split_guard": bool(nucleus / max(total, 1e-6) > .5), "phones": phones}
