...
class MultiTeacherIdentityEncoder(nn.Module):
    """Frozen-teacher projections into one shared identity/style space."""
    def __init__(self, dim: int, shared_dim: int = 32):
        super().__init__()
        self.fish_projection = nn.Sequential(nn.Linear(1024, dim), nn.SiLU(), nn.Linear(dim, shared_dim))
        self.moss_projection = nn.Sequential(nn.Linear(768, dim), nn.SiLU(), nn.Linear(dim, shared_dim))
        self.student_projection = nn.Sequential(nn.Linear(160, dim), nn.SiLU(), nn.Linear(dim, shared_dim))
        self.shared_to_dim = nn.Linear(shared_dim, dim)

    def forward(self, fish: torch.Tensor, moss: torch.Tensor) -> torch.Tensor:
        return torch.nn.functional.normalize(self.fish_projection(fish) + self.moss_projection(moss), dim=-1)

    def student(self, reference: torch.Tensor) -> torch.Tensor:
        return torch.nn.functional.normalize(self.student_projection(reference), dim=-1)
...
class TriSingerModel(nn.Module):
    def __init__(self, dim: int = 192, latent_dim: int = 768):
        ...
        self.teacher_identity_encoder = MultiTeacherIdentityEncoder(dim)
    ...
    def condition(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if "teacher_identity" in batch:
            condition = condition + self.teacher_identity_encoder.shared_to_dim(batch["teacher_identity"])[:, None, :]
    ...
    def student_identity(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        return self.teacher_identity_encoder.student(batch["reference_features"])
