...
class QualityPitchController:
    ...
    def __init__(self, checkpoint: str | Path, reference_features: torch.Tensor, device: str | None = None):
        ...
        self.version = saved.get("version", "v0.4")
    ...
    def predict(self, score: dict) -> tuple[torch.Tensor, float]:
        ...
        if self.version == "v0.5":
            ...
            residual = torch.tanh(output["pitch_residual"][0] / max(self.max_semitones, 1e-6)) * self.max_semitones
            return residual * self.residual_scale, duration
        # v0.4 compatibility: preserve residual-flow sampling behavior.
        residual = self.model.sample(batch, steps=8)[0, :, 0]
        return torch.tanh(residual) * self.max_semitones, duration
