...
Current run: teacher loss coefficient `0.15`; one teacher representation update per acoustic update. Stage plan is A teacher representation, B pseudo acoustic, C real-anchor adaptation, D joint low-LR refinement. Actual 1,200-step run is C/D bootstrap: 60 real rows with teacher representation loss. It is not a completed staged training programme; Stage B awaits licensed pseudo singing.
