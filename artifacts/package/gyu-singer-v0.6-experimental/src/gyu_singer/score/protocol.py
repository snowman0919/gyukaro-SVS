...
def normalize_score(score: dict) -> dict:
    ...
    result["style"] = {
        "preset": style.get("preset", "neutral"),
        "prosody_strength": float(style.get("prosody_strength", 1.0)),
        "acoustic_style_strength": float(style.get("acoustic_style_strength", 1.0)),
    }
    if not 0 <= result["style"]["prosody_strength"] <= 2 or not 0 <= result["style"]["acoustic_style_strength"] <= 2:
        raise ValueError("style strengths must be in [0, 2]")
