...
Status: objective PASS; human listening PASS on 2026-07-15. Approved for RC5 packaging, not final `v1.0.0`.
...
The aggregate HF-energy p95 is slightly higher on the core four (+0.00072), concentrated in rapid KO. The corrected Korean obstruent mask initially collapsed the 64-step large-interval decode; a controlled same-input sweep selected 32 steps, CFG 2.0, seed 21 with exact “높이 날아” ASR and 17.78-cent pitch MAE. Human listening confirmed candidate4 is acceptable and clearly preferable to RC4. Automated metrics did not substitute for this decision.
...
Remaining risks: rapid KO retains elevated HF energy; EN ASR says “Sink” for “Sing”; sustained “아” is transcribed as “아멘” although lyric coverage is complete. These remain explicit RC5 limitations. Final `v1.0.0` remains a separate release decision.
...
These probes did not change candidate4. Human review covered immutable hashes recorded in `human_review.json`.
...
The exposed `gyu-singer-rc5` backend reproduces all four core approved outputs byte-for-byte. RC5 packaging follows this approval; final `v1.0.0` is not tagged or released.
